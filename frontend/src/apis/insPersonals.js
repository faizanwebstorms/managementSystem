...
export const insPersonalsListingApi = async (
  model,
  searchTerm,
  page,
  limit,
  ...
  try {
    ...
    if (model) queryParams.append("model", model); // Add pagination page
    if (searchTerm) queryParams.append("searchTerm", searchTerm); // Add pagination page
    ...
    const response = await axios.get(
      `${API_BASE_URL}/v1/users/accounts/all?${queryParams.toString()}`,
      { headers }
    );
...
export const addInsPersonalApi = async (
  data,
  authToken
  // searchTerm
) => {
  try {
    ...
    const response = await axios.post(
      `${API_BASE_URL}/v1/users/account/add`,
      data,
      { headers }
    );
    console.log("response bookings", response);
